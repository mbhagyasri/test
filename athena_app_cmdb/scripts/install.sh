...
if ! [ "$(helm upgrade --install athena-app-cmdb-nginx ./helm/charts/nginx \
  -n cdk-athena-app-cmdb -f ./helm/charts/nginx/Values.yaml -f ./values/nginx/Values."${region}-${IQR_ENVIRONMENT}".yaml \
...
if ! [ "$(helm upgrade --install athena-app-cmdb ./helm/charts/cmdb \
  -n cdk-athena-app-cmdb -f ./helm/charts/cmdb/Values.yaml -f ./values/cmdb/Values."${region}-${IQR_ENVIRONMENT}".yaml \
  --set domain="${APP_DOMAIN}" --set location_id="location-${region}-${IQR_ENVIRONMENT}" \
